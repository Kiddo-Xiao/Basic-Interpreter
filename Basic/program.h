 ...
 */
   ...
   void run(EvalState &a);
   void list();

private:
    struct proline{
        string old_line;
        Statement *new_line=nullptr;
        proline()=default;//初始不赋初值构造不被下两个重载所覆盖，proline a合法
        proline(string &a):old_line(a){}
        proline(string &a,Statement *b):old_line(a),new_line(b){}
    };

    map<int,proline> allpro;
